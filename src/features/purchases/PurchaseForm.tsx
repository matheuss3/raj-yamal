...
export function PurchaseForm({ tags, onSubmit }: PurchaseFormProps) {
  ...
  return (
    <form onSubmit={handleFormSubmit} style={{ display: "flex", flexDirection: "column", gap: "1.25rem" }}>
      ...
      <div key={step} style={{ animation: `${direction === "forward" ? "step-in-forward" : "step-in-back"} 220ms var(--ease-standard)` }}>
        {step === "description" && (
        ...
        {step === "confirm" && (
          <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
            ...
            >
              ...
              <SummaryRow label="Valor" value={formatCentsToBRL(amountCents)} />
...
function SummaryRow({ label, value }: { label: string; value: string }) {
  return (
    <div style={{ display: "flex", justifyContent: "space-between", gap: "0.75rem" }}>
      ...
      <span style={{ textAlign: "right", overflowWrap: "break-word", minWidth: 0 }}>{value}</span>
    </div>
  );
}
